...
const REQUIRED_KEYS := ["role", "branch", "type", "tier", "cost", "prereqs",
						"level_req", "description", "passive_key", "passive_val", "cooldown"]
...
func test_total_node_count_is_22() -> void:
	assert_eq(SkillDefs.ALL.size(), 22)

func test_fighter_node_count_is_11() -> void:
	assert_eq(SkillDefs.get_nodes_for_role("Fighter").size(), 11)
...
func test_get_def_returns_correct_dict() -> void:
	var d: Dictionary = SkillDefs.get_def("f_headshot")
	assert_eq(d["role"], "Fighter")
	assert_eq(int(d["tier"]), 1)
	assert_eq(d["passive_key"], "headshot_mult")
...
func test_get_branches_for_fighter() -> void:
	...
	assert_true(branches.has("Combat"))
	assert_true(branches.has("Mobility"))
	assert_true(branches.has("Resilience"))
...
func test_get_nodes_in_branch_sorted_by_tier() -> void:
	var nodes: Array = SkillDefs.get_nodes_in_branch("Fighter", "Combat")
