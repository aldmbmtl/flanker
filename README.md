...
└── scripts/
    ...
    ├── SkillTree.gd         # autoload — skill tree state, RPC, cooldowns, passive bonus queries
    ├── SkillDefs.gd         # autoload — static skill definitions
    ├── FighterSkills.gd     # Fighter skill execution
...
Current baseline: **591 passing** and **8 pending/risky** (all intentional — documented known bugs or no-assert smoke tests). Any run dropping below 591 passing is a regression.
