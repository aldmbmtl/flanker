...
func _on_assign_active_requested(node_id: String) -> void:
	# Show a small popup to pick slot 0 (Q) or 1 (E)
	# Simple approach: assign to first empty slot, otherwise slot 0
	var slots: Array = SkillTree.get_active_slots(_peer_id)
	var target_slot: int = 0
	if slots[0] != "" and slots[1] == "":
		target_slot = 1
	if _is_mp and not multiplayer.is_server():
		SkillTree.request_assign_active.rpc_id(1, target_slot, node_id)
	else:
		SkillTree.assign_active_slot(_peer_id, target_slot, node_id)
